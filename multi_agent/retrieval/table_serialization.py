...
def _rows_to_documents(rows: list[list], table_name: str, source: str,
    ...
    for i, row in enumerate(rows[1:], start=0):
        ...
        # Track hierarchical category section headers (e.g., "Non-current assets")
        # A section header row has a populated label in the first cell, but all other columns are empty.
        if len(clean_cells) > 1 and clean_cells[0] and not any(clean_cells[1:]):
        ...
        # Subject = first non-empty cell, with its header stripped from the
        # sentence body so we never repeat it.
        subject_col = None
        subject_val = ""
        ...
        full_subject = f"{current_section} - {subject_val}" if current_section else subject_val
